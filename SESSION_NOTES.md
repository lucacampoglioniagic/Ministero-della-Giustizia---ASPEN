...
### Bugfix #2 — grafico "Esoneri Attivi" ancora vuoto dopo il fix del fetchxml (11/09/2026, stessa sessione)
Dopo il fix precedente il grafico restava comunque invisibile (canvas presente nel DOM ma bloccato alle dimensioni HTML di default 300×150px, nessuna barra visibile), nonostante i dati corretti arrivassero al browser (verificato via Network tab e query Web API dirette). Diagnosi eseguita in live debugging con Playwright (navigazione autenticata diretta alla dashboard, ispezione DOM/canvas, lettura bundle.js effettivamente servito):
- **Falsa pista iniziale**: sospettato un problema di timing Chart.js (canvas che passa da `display:none` a `display:block` nello stesso tick sincrono di `new Chart(...)`, con lettura di una dimensione del contenitore ancora a zero). Applicata mitigazione (creazione del chart e resize rimandati a `requestAnimationFrame`) ma il canvas restava comunque a 300×150 dopo il redeploy.
- **Causa reale**: aggiunta un'istrumentazione di debug temporanea (`window.__esoneriDebug`, poi try/catch con cattura dello stack) per verificare se `updateView` venisse effettivamente eseguito. Risultato: `record.getValue("dataInizioField")` non restituisce sempre un oggetto `Date` nativo in questo contesto (dataset PCF su dashboard), causando l'eccezione silenziosa `inizio.getTime is not a function` — silenziosa perché il PCF host di Dynamics non la logga in console, la inghiotte semplicemente interrompendo l'esecuzione di `updateView` prima che il canvas venga reso visibile (`style.display` restava impostato al default vuoto per sempre, ad ogni chiamata successiva).
- **Fix**: introdotta una funzione di utilità `toDateOrNull()` che converte in modo sicuro il valore restituito da `getValue()` in `Date` (gestendo sia il caso in cui sia già un `Date` sia il caso in cui sia una stringa/numero), usata sia per `dataInizioField` sia per `dataFineField` in `isEsoneroEffettivamenteAttivo()`. Rimossa tutta l'istrumentazione di debug temporanea al termine dell'indagine; mantenuto solo il fix legittimo del timing Chart.js (`requestAnimationFrame` su creazione/resize del grafico) come buona pratica difensiva.
- **Nota tecnica sulla cache client**: durante l'indagine si è anche scoperto che il client Dynamics 365 mantiene una cache dei web resource con URL "version-stamped" (es. `/{000000004588153}/webresources/...`) che **non viene invalidata automaticamente da `PublishAllXml`**; una semplice ricarica della pagina può continuare a servire una versione precedente del bundle.js. Per forzare il caricamento della versione più recente durante il test è stato necessario cancellare IndexedDB/localStorage/Cache Storage del browser prima del reload.
- Rebuild e redeploy di `EsoneriAttiviChart` (stesso workaround: spostamento temporaneo cartelle "fratelli" + `pac pcf push` + import manuale dello zip generato). Verificato via Playwright: canvas correttamente dimensionato (1060×538px, non più 300×150), barre visibili con i colori corretti (Totale=rosso, Parziale=arancio), screenshot di conferma acquisito.

### Bugfix #3 — dati sbagliati nel grafico "Esoneri Attivi": mancavano i magistrati con esonero Totale (11/09/2026, stessa sessione)
Dopo il fix #2 il grafico si vedeva ma l'utente ha segnalato che i dati non tornavano: nella vista "Esoneri attivi/e" risultavano attivi anche Chiara Marini (esonero Totale, "Ferie") e Alessia Gialli (esonero Totale, "test"), ma il grafico mostrava solo Anna Greco (Parziale, 10%). **Causa**: per gli esoneri di tipo **Totale** il campo `agc_percentualeesonero` non viene valorizzato (coerente con `agc_assignfascicolo.js`, che ignora la percentuale per il tipo Totale — l'esonero è per definizione al 100%); il codice leggeva `Number(record.getValue("percentualeField")) || 0`, ottenendo `0` per questi record e quindi una barra di altezza nulla (il magistrato compariva come etichetta ma senza barra visibile). **Fix**: in `EsoneriAttiviChart/index.ts`, per il tipo Totale la percentuale viene ora forzata a `100` invece di leggere il campo. Verificato via Playwright: tutti e 3 i magistrati con esonero attivo (Chiara Marini 100%, Alessia Gialli 100%, Anna Greco 10%) ora visibili con barre corrette.
